...
mouse = 'IS002'  # 'BS003'
nameSession = 'IS002_10042021_g0'
protocol = 'TimeInvestBernoulli' #'TonicDA' #'TimeInvestBernoulli'  # 'BeliefState'
...
# Input data
# Name for log file for this pipeline run. Log file will be saved in the output destination directory catGT_dest
logName = 'pipeline_log.csv'  # If this file exists, new run data is appended to it

# Raw data directory (npx_directory)
npx_directory = r'C:\Users\sromeropinto\Documents\Data\SpikesToProcess'
json_directory =  os.path.join(r'C:\Users\sromeropinto\Documents\Data\ecephys_spike_sorting\json_files', mouse)
src = os.path.join(r'Z:\Sandra\Data', protocol, 'Spikes', mouse)
if not os.path.isdir(json_directory):
    os.mkdir(json_directory)
# Output destination
# All output will be written here:
catGT_dest = os.path.join(r'C:\Users\sromeropinto\Documents\Data\ecephys_spike_sorting\cat_outputs', mouse)
if not os.path.isdir(catGT_dest):
    os.mkdir(catGT_dest)
print('Processing' + nameSession)
dest = npx_directory
srcSession = os.path.join(src, nameSession)
destSession = os.path.join(dest, nameSession)

if nameSession not in os.listdir(dest):
    # Copying session
    print('Copying ' + srcSession + ' to ' + destSession)
    destination = shutil.copytree(srcSession, destSession)
    print(srcSession + 'copied!!')
else:
    print('Session already copied locally')

# Each run_spec is a list of 4 strings:
#   1. Undecorated run name (no g/t specifier, the run field in CatGT)
#   2. Gate index, as a string (e.g. '0')
...
#   (must match a key in the param dictionaries above.)

run_specs = [[nameSession[:-3], '0', '0,0', '0', ['cortex']]]
print('Run specs: ' + run_specs[0][0])

# ------------------------------------------------------------------------------------------

# --------------------------------------------------------------------------------------------------------------
# CatGT params
car_mode = 'gblcar'  # CAR mode for CatGT. Must be equal to 'None', 'gbldmx', 'gblcar' or 'loccar'
loccar_min = 40  # inner and outer radii, in um for local comman average reference, if used
loccar_max = 160
...
ks_saveRez = 1
ks_copy_fproc = 0
ks_templateRadius_um = 163  # Template radius and whitening (um) : translated into sites using the probe geometry
ks_whiteningRadius_um = 163
ks_minfr_goodchannels = 0.1

# --------------------------------------------------------------------------------------------------------------
# C_Waves
c_Waves_snr_um = 160  # snr radius (um)

# --------------------------------------------------------------------------------------------------------------
# Psth_events parameters
# Extract param string forevents: copy CatGT params used to extract events.
# should be exported with the phy output for PSTH plots
...
if not os.path.isfile(logFullPath):
    log_from_json.writeHeader(logFullPath)  # create the log file, write header
...
for spec in run_specs:
    ...
    # to determine the range of trials if the user specified t limits as start and end
    ...
    # loop over all probes to build json files of input parameters initalize lists for input and output json files
    catGT_input_json = []
    catGT_output_json = []
    module_input_json = []
    module_output_json = []
    ...
    # Remove the folder I copied
    shutil.rmtree(destSession)
    # Move the folder with outputs to cluster
    outputSession = os.path.join(catGT_dest, 'catgt_' + nameSession)
    outputDestSession = os.path.join(srcSession, 'catgt_' + nameSession)
    print('Moving from ' + outputSession + ' to ' + outputDestSession)
    destination = shutil.move(outputSession, outputDestSession)
